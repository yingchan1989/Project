...
#Code purpose: run a deep reinforcement learning network (Q-learning) with neural networks to allow the agent to play Gomoku after self play
...
def getRealReward(grid, color_id):
    reward = 0
    if color_id == 1:
        my_max_length = get_state(grid, 1)[:,1]
        other_max_length = get_state(grid, -1)[:,1]
...
def make_action(grid, action, color_id):
    ...
    full_state = get_full_state(grid, color_id)

    avail_left = full_state[action, 1]
    avail_right = full_state[action, 2]
    avail = []
    if avail_left != -1:
...
def make_simple_action(grid, action, color_id):
    import random
    #copy grid into a new grid
    new_grid = []
    for i in range(0, len(grid)):
...
model = Sequential()
model.add(Dense(500, init='lecun_uniform', input_shape=(3*(board_size + board_size)*2 + 3*(board_size-4+board_size-5)*2*2,)))
...
number_of_actions = (board_size + board_size) * 2 +  (board_size - 4 + board_size - 5) * 2 * 2
model.add(Dense(number_of_actions, init='lecun_uniform')) #limit to two outputs, one is attack and the other will be defend
...
epsilon = 0.5  #can play around with this variable
...
for i in range(epochs):
    ...
    while(status == 1):
        ...
        if (j % 2 == 0):  # even
            ...
            qval = model.predict(
                state.reshape(1, 3 * (board_size + board_size) * 2 + 3 * (board_size - 4 + board_size - 5) * 2 * 2),
                batch_size=1)
            ...
            # Get max_Q(S',a)
            newQ = model.predict(
                new_state.reshape(1, 3 * (board_size + board_size) * 2 + 3 * (board_size - 4 + board_size - 5) * 2 * 2),
                batch_size=1)
            maxQ = np.max(newQ)
            ...
            model.fit(
                state.reshape(1, 3 * (board_size + board_size) * 2 + 3 * (board_size - 4 + board_size - 5) * 2 * 2), y,
                batch_size=1, nb_epoch=1, verbose=1)

            # Get the new state and grd and print the grid

        else:
            color_id = -1
            list_of_zeros = [abc for abc, x in enumerate(grid) if x == 0]
            random_stone = random.choice(list_of_zeros)
            new_grid = grid[:]
            new_grid[random_stone] = color_id
            new_state = get_full_state(new_grid, color_id)
            reward = getReward(new_grid, color_id)
        ...
        if reward != 0 or len([id for id, x in enumerate(grid) if x == 0]) == 0:
            status = 0

    if epsilon > 0.1:
        epsilon -= (10 / epochs)

epsilon = 0.8  #can play around with this variable
epochs = 10000
...
for i in range(epochs):
    ...
    while(status == 1):
        ...
        if (j % 2 == 0):  # even
            ...
            print("Opponent Random Action : %s" % (action,))
            new_grid = make_action(grid, action, color_id)
            new_state = get_full_state(new_grid, color_id)
            reward = getReward(new_grid, color_id)
...
def predict_next_move(prev_grid, color_id):
    ...
    pred_q = model.predict(state.reshape(1, 3*(board_size + board_size)*2 + 3*(board_size-4+board_size-5)*2*2), batch_size=1)
...
#Start game against a random player
no_games = 100
wins = 0
loss = 0
draw = 0
for i in range(no_games):
    ...
    while(status == 1):
        ...
        one_reward = getRealReward(grid, 1)
        neg_reward = getRealReward(grid, -1)

        if one_reward == 1000:
            status = 0
            wins += 1
        elif one_reward == -1000:
            status = 0
            loss += 1
        elif len([i for i, x in enumerate(grid) if x == 0]) == 0:
            status = 0
            draw += 1
        #elif one_reward == 10:
        #    status = 0
...
print "Random spaces player:"
print wins
print loss
print draw

#Random action player evaluation
no_games = 100
wins = 0
loss = 0
draw = 0
for i in range(no_games):
    ...
    while(status == 1):
        if j % 2 == 0:
            ...
            action = np.random.randint(0, number_of_actions)
            grid = make_action(grid, action, -1)
